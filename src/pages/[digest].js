...
const Article = ({ entry }) => {
	...
	const { mailingListURL } = JSON.parse(entry.publication?.publicationSettings?.settings || '{ "mailingListURL": null }')
		...
		<Layout publication={entry.publication}>
			<article>
				...
				<footer className="border dark:border-gray-800 rounded-lg divide-y dark:divide-gray-800 font-mono max-w-xl mx-auto">
					{entry.arweaveTransactionRequest && (
...
export async function getStaticProps({ params: { digest } }) {
	...
	const darkMode = JSON.parse(entry.publication?.publicationSettings?.settings || '{ "darkMode": false }').darkMode
	...
	return {
		props: { entry: { ...entry, body: String(body) } },
